...
/** Production PayPal wordmark from `PayPalButton-Bjp1Yiiy.js` (6 paths). */
function PayPalMark() {
  return (
    <svg
      width="80"
...
export function PayPalBuyNowButton({
  ...
  return (
    <button
      ...
      className="flex w-full cursor-pointer items-center justify-center gap-2 rounded-lg text-base font-semibold transition-opacity hover:opacity-90 disabled:cursor-not-allowed disabled:opacity-60"
      style={{ backgroundColor: '#FFC439', color: '#003087', height: '48px', border: 'none' }}
    >
      {submitting ? (
        <>
          <div className="h-5 w-5 animate-spin rounded-full border-2 border-[#003087]/30 border-t-[#003087]" />
          <span style={{ color: '#003087', fontSize: '13px', fontWeight: 600 }}>Redirecting to PayPal...</span>
        </>
      ) : (
        <>
          <PayPalMark />
          <span style={{ color: '#111' }}>Buy Now</span>
        </>
      )}
    </button>
  )
...
function PayPalButtonInner({
  ...
  onBeforeRedirect,
  stripe,
}: PayPalButtonProps & { stripe: StripeWithPayPal | null }) {
  const createPayPalPaymentIntent = useCreatePayPalPaymentIntent()
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      onError?.('PayPal redirect is not available. Please try again or pay with card.')
    } catch (error) {
...
function PayPalButtonWithStripe(props: PayPalButtonProps) {
  const stripe = useStripe() as StripeWithPayPal | null
  return <PayPalButtonInner {...props} stripe={stripe} />
}

export default function PayPalButton(props: PayPalButtonProps) {
  if (!isStripeConfigured) {
    return <PayPalButtonInner {...props} stripe={null} />
  }

  return (
    <Elements stripe={getStripe()} options={{ appearance: { theme: 'stripe' } }}>
      <PayPalButtonWithStripe {...props} />
    </Elements>
  )
}
